...
def decomposed_mse(evaluation, simulation):
    ...
    if len(evaluation) == len(simulation):
        ...
        sdsd = (e_std - s_std) ** 2
        lcs = 2 * e_std * s_std * \
            (1 - correlationcoefficient(evaluation, simulation))
...
def _spearmann_corr(x, y):
    ...
    numerator = np.nansum(
        [float((a[j][2] - MW_rank_x) * (a[j][3] - MW_rank_y)) for j in range(len(a))])
    denominator1 = np.sqrt(
        np.nansum([(a[j][2] - MW_rank_x) ** 2. for j in range(len(a))]))
    denominator2 = np.sqrt(
        np.nansum([(a[j][3] - MW_rank_x) ** 2. for j in range(len(a))]))
...
def kge_non_parametric(evaluation, simulation, return_all=False):
    ...
    if len(evaluation) == len(simulation):
        ...
        fdc_sim = np.sort(
            simulation / (np.nanmean(simulation) * len(simulation)))
        fdc_obs = np.sort(
            evaluation / (np.nanmean(evaluation) * len(evaluation)))
...
def calculate_all_functions(evaluation, simulation):
    ...
    for f in _all_functions:
        ...
        try:
            x = f(evaluation, simulation)
            if(np.isnan(x)):
                result.append((f.__name__, 0))
            else:
                result.append((f.__name__, x))
            text = f.__name__ + " = "
